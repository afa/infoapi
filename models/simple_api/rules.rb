  ...
  class Rules
    class << self
      ...
      def process(params, sphere, logger)
        ...
        logger.info "for sphere #{sphere} with params #{params.inspect} selected rule #{found.is_a?(Array) ? found.first.try(:id) : found.id} #{found.is_a?(Array) ? found.first.try(:name) : found.name}."
