  ...
  class Index
    class << self
      ...
      def index_links(curr, route, param, range)
        ...
        # if curr[:id]
        p 'indexlinks'
        SimpleApi::Sitemap::ObjectData.select(:object_data_items__id, :object_data_items__index_id, Sequel.function(:random).as(:random), :object_data_items__photo, :refs__url, :refs__rule_id, :refs__json).distinct(:object_data_items__index_id).join(:indexes, indexes__id: :object_data_items__index_id).join(:refs, indexes__id: :refs__index_id).where(refs__is_empty: false, refs__super_index_id: curr.pk).offset(range.first).limit(range.size).all.map do |ref|
